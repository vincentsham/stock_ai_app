...
def transform_records(conn, tic: str, date: str) -> pd.DataFrame:
    ...
    df_cash_flow['net_common_stock_issuance_ttm'] = -df_cash_flow['net_common_stock_issuance'].rolling(window=4).sum()
    df_cash_flow['net_common_dividends_paid_ttm'] = -df_cash_flow['common_dividends_paid'].rolling(window=4).sum()
    ...
    df['market_cap_asof'] = df['close_price'] * df['shares_outstanding']
