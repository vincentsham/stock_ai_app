...
def transform_records(conn, tic: str, date: str) -> pd.DataFrame:
    ...
    pe_forward = current_price / pe_forward if pe_forward and pe_forward > 0 and current_price else None
    df_earnings.loc[df_earnings.index[-2:], 'eps_est_ttm_forward'] = pe_forward
    # df_earnings.loc[df_earnings.index[-4:], 'eps_est_ttm_forward'] = pe_forward
    ...
    df_earnings.loc[df_earnings.index[-1], 'revenue_est_ttm_forward'] = revenue_est_forward
    ...
    df_earnings['forward_eps_growth'] = df_earnings.apply(
        ...
        if row['eps_ttm'] and row['eps_ttm'] > 0 else None, axis=1
    )
    ...
    df['operating_income_growth_yoy'] = df.apply(
        ...
        if row['operating_income_ttm_prev'] and row['operating_income_ttm_prev'] > 0 else None, axis=1
    )
