...
def transform_records(conn, tic: str, date: str) -> pd.DataFrame:
    ...
    df['net_debt'] = df['total_debt'] - df['cash_and_short_term_investments']
    df['net_debt_to_ebitda_ttm'] = df.apply(
        lambda row: float(row['net_debt']) / float(row['ebitda_ttm'])
        if row['ebitda_ttm'] and row['ebitda_ttm'] != 0 else None, axis=1
    )
    df['interest_coverage_ttm'] = df.apply(
        lambda row: float(row['ebit_ttm']) / float(row['interest_expense_ttm'])
        if row['interest_expense_ttm'] and row['interest_expense_ttm'] != 0 else None, axis=1
    )
    df['current_ratio'] = df.apply(
        lambda row: float(row['total_current_assets']) / float(row['total_current_liabilities'])
        if row['total_current_liabilities'] and row['total_current_liabilities'] != 0 else None, axis=1
    )
    ...
    df['debt_to_equity'] = df.apply(
        ...
        if row['total_stockholders_equity'] and row['total_stockholders_equity'] > 0 else None, axis=1
    )
    df['debt_to_assets'] = df.apply(
        lambda row: float(row['total_debt']) / float(row['total_assets'])
        if row['total_assets'] and row['total_assets'] != 0 else None, axis=1
    )

    df['A'] = (df['total_current_assets'] - df['total_current_liabilities']) / df['total_assets']
    df['B'] = df['retained_earnings'] / df['total_assets']
    df['C'] = df['ebit_ttm'] / df['total_assets']
    df['D'] = df['total_stockholders_equity'] / df['total_liabilities']
    df['E'] = df['revenue_ttm'] / df['total_assets']

    df['altman_z_score'] = (1.2 * df['A']) + (1.4 * df['B']) + (3.3 * df['C']) + (0.6 * df['D']) + (1.0 * df['E'])

    df['cash_runway_months'] = df.apply(
        lambda row: float(row['total_liabilities']) / (abs(float(row['fcf_ttm'])) / 12)
        if pd.notna(row['total_liabilities']) and pd.notna(row['fcf_ttm']) and row['fcf_ttm'] < 0 else None, axis=1
    )  
