...
def transform_records(conn, tic: str, date: str) -> pd.DataFrame:
    ...
    val_to_assign = current_price / pe_forward if pe_forward and pe_forward > 0 and current_price else None
    df_earnings.loc[df_earnings.index[-2:], 'eps_est_ttm_forward'] = val_to_assign
    ...
    df['pe_forward'] = df.apply(
        ...
        if row['eps_est_ttm_forward'] is not None and row['eps_est_ttm_forward'] > 0 else None, axis=1
    )
    ...
    df['peg_ratio'] = df.apply(
        lambda row: float(row['pe_ttm']) / float(row['eps_growth_rate'])
    ...
    df['earnings_yield_ttm'] = df.apply(
        lambda row: float(row['eps_ttm']) / float(row['close_price'])
        if row['close_price'] and row['close_price'] > 0 and pd.notna(row['eps_ttm']) else None, axis=1
    )
