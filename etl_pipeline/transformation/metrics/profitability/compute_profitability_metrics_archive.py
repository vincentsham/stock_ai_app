...
def transform_records(conn, tic: str, date: str) -> pd.DataFrame:
    ...
    df_balance_sheet['total_stockholders_equity_avg'] = (df_balance_sheet['total_stockholders_equity'] + df_balance_sheet['total_stockholders_equity'].shift(4))
    ...
    df['roe'] = df.apply(
        lambda row: float(row['net_income_ttm']) / float(row['total_stockholders_equity'])
