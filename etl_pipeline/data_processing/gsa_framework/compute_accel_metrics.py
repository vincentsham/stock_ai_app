...
def compute_accel_metrics(df: pd.DataFrame, column: str) -> pd.DataFrame:
    """
    Compute all acceleration metrics for a specified column.
    """
    df = calculate_acceleration(df, column)
    df = calculate_accel_count_4q(df, column)
    df = calculate_accel_streak_length(df, column)
    df = compute_accel_regime(df, column)
    return df