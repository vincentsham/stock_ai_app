...
def main(tic: str, calendar_year: int = 2024, calendar_quarter: int = 4, sleep_time=125):
    ...
    if conn:
        if calendar_year is None or calendar_quarter is None:
            ...
            LIMIT 1;
            """
            latest_df = read_sql_query(query_latest_year_quarter, conn)
            if not latest_df.empty:
                calendar_year = int(latest_df.loc[0, 'calendar_year'])
                calendar_quarter = int(latest_df.loc[0, 'calendar_quarter'])
                yq_filter = f"AND (et.calendar_year, et.calendar_quarter) > ({calendar_year}, {calendar_quarter})"
            else:
                yq_filter = ""
            ...
            query = f"""
                ...
                ON et.tic = sm.tic
                WHERE et.tic = '{tic}'
                    AND (et.calendar_year, et.calendar_quarter) = ({calendar_year}, {calendar_quarter})
                    AND et.transcript_sha256 IS DISTINCT FROM eta.transcript_sha256;
            """
    ...
    for state in tqdm(states, desc=f"Processing earnings transcripts - {tic}"):
        while retries > 0:
            try:
...
if __name__ == "__main__":
    ...
    if conn:
        ...
        for record in records:
            tic = record[0]
            main(tic=tic, calendar_year=None, calendar_quarter=None, sleep_time=125)
