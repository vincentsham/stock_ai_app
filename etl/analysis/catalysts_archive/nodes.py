...
def current_catalysts_retriever_node(state: CatalystSession) -> dict:
    ...
    earliest_date = None
    for ctx in state.catalysts:
        chunk_date = ctx.chunk.date
        if earliest_date is None or chunk_date < earliest_date:
            earliest_date = chunk_date
    date_filter = f"AND date::DATE >= '{earliest_date}'::DATE - INTERVAL '6 months'"
    ...
    sql = f"""
        ...
        WHERE tic = '{tic}' 
              AND catalyst_type = '{catalyst_type}'
              {date_filter if earliest_date else ''}
        ORDER BY date DESC
        LIMIT 5;
    """
