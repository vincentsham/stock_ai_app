...
def main():
    ...
    if conn_local and conn_supabase:
        ...
        try:
            total_deleted = delete_published_records(conn_local, "mart.profitability_metrics", today, commit=False)
            delete_published_records(conn_supabase, "mart.profitability_metrics", today, commit=False)
            ...
            for record in records:
                ...
                df = df[cols]
                total_inserted = insert_records(conn_local, df, "mart.profitability_metrics", 
                                            ["tic", "date", "as_of_date"], 
                                            updated_at=False, commit=False)
                insert_records(conn_supabase, df, "mart.profitability_metrics", 
                                            ["tic", "date", "as_of_date"], 
                                            updated_at=False, commit=False)
