            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
              ...
              <div className="flex justify-center w-auto mx-auto items-center">
                ...
                {['Trend', 'Growth', 'Acceleration'].map((tab) => (
                    <button
                        key={tab}
                        onClick={() => setViewMode(tab.toLowerCase() as 'trend' | 'growth' | 'acceleration')}
              ...
              <div className="bg-[#111218] p-5 rounded-xl border border-gray-800 hover:border-gray-700 transition-colors">
                <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-6 gap-4">
                  <h3 className="font-bold text-gray-200 flex items-center gap-2 text-sm w-30">
                    {viewMode === 'trend' ? 'EPS Trend' : viewMode === 'growth' ? 'EPS YoY Growth' : 'EPS YoY Acceleration'}
                  </h3>
                  {/* EPS Summary Labels */}
                ...
                {viewMode === 'trend' && <EarningsGraph data={chartData} metric="eps" />}
                {viewMode === 'growth' && <EarningsGrowthGraph data={chartData} metric="eps" type='growth' />}
                {viewMode === 'acceleration' && <EarningsGrowthGraph data={chartData} metric="eps" type='acceleration' />}
              </div>
              ...
              <div className="bg-[#111218] p-5 rounded-xl border border-gray-800 hover:border-gray-700 transition-colors">
                <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-6 gap-4">
                  <h3 className="font-bold text-gray-200 flex items-center gap-2 text-sm w-40">
                    {viewMode === 'trend' ? 'Revenue Trend' : viewMode === 'growth' ? 'Revenue YoY Growth' : 'Revenue YoY Acceleration'}
                  </h3>
                ...
                {viewMode === 'trend' && <EarningsGraph data={chartData} metric="revenue" />}
                {viewMode === 'growth' && <EarningsGrowthGraph data={chartData} metric="revenue" type='growth' />}
                {viewMode === 'acceleration' && <EarningsGrowthGraph data={chartData} metric="revenue" type='acceleration' />}
              </div>
